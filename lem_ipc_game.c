...
bool next_to_target(t_ipc *ipc){
    int dx[] = {-1, -1,  0, 1, 1, 1,  0, -1};
    int dy[] = { 0,  1,  1, 1, 0,-1, -1, -1};

    for (int i = 0; i < 8; i++){
        int pos_next_x = ipc->targets[data_player.team - 1].pos_x + dx[i];
        int pos_next_y = ipc->targets[data_player.team - 1].pos_y + dy[i];
...
void do_move(t_ipc *ipc, int team){
    ...
    for (int i = 0; i < 8; i++){
        int target_x = ipc->targets[data_player.team - 1].pos_x + dx[i];
        int target_y = ipc->targets[data_player.team - 1].pos_x + dy[i];
...
void do_game(t_ipc *ipc){
    ...
    while(game_over(ipc) == false){
        if (ipc->playing_player == data_player.index){
            ...
            msgrcv(ipc->teams[data_player.team - 1].id_msg, msg_recv, sizeof(t_msg), 0, 0);
        }
